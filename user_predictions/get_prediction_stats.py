...
label_and_table_names.append(('african american', 
                              'user_px5_african_american_100k_updated'))
...
labels = []
values = [] # total ct, non-null ct, pos ct, neg ct, pos proportion
...
for label, table_name in label_and_table_names:
    ...
    sys.stdout.flush()
    query = """
            SELECT total, non_null, positive, negative,
                   positive/non_null::decimal AS proportion_positive
            FROM 
    ...
    labels.append(label)
    values.append((int(rows[0][0]), int(rows[0][1]), int(rows[0][2]), 
                   int(rows[0][3]), float(rows[0][4])))
sys.stdout.write('\n')
