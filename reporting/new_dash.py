...
def handle_request():
    ...
    try:
        ...
        _today = open('{0}/client_{1}_all_campaigns_day.txt'.format(read_from,client_id),'r')
        _today_data = _today.read()
        _today.close()
	aggregate = open('{0}/client_{1}_all_campaigns_aggregate.txt'.format(read_from,client_id),'r')
        aggregate_data = aggregate.read()
        aggregate.close()

	hourly_aggregate = open('{0}/client_{1}_hourly_aggregate.txt'.format(read_from,client_id),'r').read()
	daily_aggregate = open('{0}/client_{1}_daily_aggregate.txt'.format(read_from,client_id),'r').read()

	f1 = open('{0}/client_{1}_data_all.txt'.format(read_from,client_id),'r')
        all_data = f1.read()
        f1.close()
	f2 = open('{0}/client_{1}_data_day.txt'.format(read_from,client_id),'r')
        day_data = f2.read()
        f2.close()
	f3 = open('{0}/client_{1}_data_hourly.txt'.format(read_from,client_id),'r')
        hourly_data = f3.read()
        f3.close()
	f4 = open('{0}/client_{1}_data_monthly.txt'.format(read_from,client_id),'r')
