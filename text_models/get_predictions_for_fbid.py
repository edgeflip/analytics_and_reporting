...
from evaluate_user_rankings import get_post_vectors, get_user_posts
...
def get_post_and_aboutme_document_from_id(fbid_in, conn):
    ...
    query = """
            ...
            GROUP BY fbid_post
            """.format(posts_table='posts_raw', fbid=fbid_in)
    ...
    return User_Posts(fbid_in, fbid_to_message[fbid_in].getvalue().strip())
...
def get_k_most_discriminative_features(fbid_user_posts, k, label):
    ...
    if label == 'vegan':
        ...
        vocabulary_filename = '{full_dir_name}/tfidf_vocabulary_{positive_label}_{negative_label}{file_suffix}.out'.format(
                                full_dir_name=full_dir_name,
                                positive_label=positive_label, 
                                negative_label=negative_label, 
                                file_suffix=file_suffix)
        idf_filename = '{full_dir_name}/idf_vector_{positive_label}_{negative_label}{file_suffix}.out'.format(
                                full_dir_name=full_dir_name, 
                                positive_label=positive_label, 
                                negative_label=negative_label, 
                                file_suffix=file_suffix)
        classifier_filename = '{full_dir_name}/linear_svc_{positive_label}_{negative_label}{file_suffix}.out'.format(
                                full_dir_name=full_dir_name, 
                                positive_label=positive_label, 
                                negative_label=negative_label, 
                                file_suffix=file_suffix)
    ...
    for i, fbid_user_post in enumerate(fbid_user_posts):
        ...
        for top_feature, top_val, bottom_feature, bottom_val in zip(top_k_features, top_k_vals, 
                                                                    bottom_k_features, bottom_k_vals):
...
label_to_files = {} # label -> (dir_name, positive_label, negative_label, dir_suffix, file_suffix)
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        sys.stderr.write('Usage: python get_predictions_for_fbid.py fbid [on_network] [top k features]\n')
        sys.exit()
    ...
    if len(sys.argv) > 4:
        ...
        posts_cache_filename = '/data/user_documents/individual_posts_marc_friends/all-individual-posts.txt'
        aboutme_cache_filename = '/data/user_documents/individual_posts_marc_friends/all-individual-aboutme.txt'
    else:
    ...
    if not on_network:
        ...
        if top_k:
            fbid_posts = get_post_and_aboutme_document_from_id(fbid, conn)
            dfs = []
            for label in label_to_table_names:
                sys.stdout.write('computing top features for {}...\n'.format(label))
                sys.stdout.flush()            
                df = get_k_most_discriminative_features([fbid_posts], top_k, label)
            ...
            prediction_with_features_df = pd.merge(prediction_df, feature_df, on=['label'])
        ...
        if top_k and not posts_from_cache:
            _, fbid_user_posts, _ = get_post_and_aboutme_document_from_ids_helper(temp_network_table_name, conn)
        elif top_k and posts_from_cache:
            fbid_user_posts = get_user_posts(posts_cache_filename, aboutme_cache_filename, None)[0]
            fbid_user_posts = [User_Posts(user_posts.fbid, user_posts.posts + ' ' + user_posts.aboutme + ' ' + user_posts.link_desc) for user_posts in fbid_user_posts]
        else:
            pass
        ...
        for label, table_name in label_to_table_names.items():
            ...
            rows = execute_query(query, conn)
            for row in rows:
                labels_and_predictions.append( (label, int(row[0]), float(row[1]), str(row[2])) )
        
        prediction_df = pd.DataFrame(data=np.array(labels_and_predictions, dtype=('object,int32,float64,object')))
        prediction_df.columns = ['label', 'fbid', 'prediction', 'name']
        prediction_df = prediction_df.sort(['label', 'prediction'], ascending=[True, False])
        ...
        if not top_k:
            for label in label_to_table_names:
                print(prediction_df[prediction_df['label'] == label].sort('prediction', ascending=False).head(10))
                print('')
        else:
            pd.set_option('display.max_rows', 500)
            pd.set_option('display.max_columns', 500)
            pd.set_option('display.width', 1000)
            prediction_with_features_df = pd.merge(prediction_df, feature_df, on=['label', 'fbid'])
            prediction_with_features_df = prediction_with_features_df.set_index(['label'])
            for label in label_to_table_names:
                print(prediction_with_features_df.ix[label].head(100))
                print('')
